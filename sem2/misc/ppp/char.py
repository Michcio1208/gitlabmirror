...
# Prąd rekombinacji w warstwie zaporowej (Dioda 1 ma szerszą warstwę -> większy prąd)
I_rek_1 = 1e-9 * np.exp(U / (2 * Vt))
I_rek_2 = 3.16e-10 * np.exp(U / (2 * Vt)) # Mniejszy prąd ze względu na mniejszą szerokość warstwy zaporowej (sqrt(10) raza mniejsza)
...
plt.plot(U, np.log10(I_tot_1), label='Dioda 1 ($N_{A1}$, $w_1=2L_n$)', color='blue', linewidth=2)
plt.plot(U, np.log10(I_tot_2), label='Dioda 2 ($10 N_{A1}$, $w_2 \approx 0.1L_n$)', color='red', linestyle='--', linewidth=2)

# Oznaczenia asympotot i nachyleń
...
plt.title('Charakterystyka $lg(I) = f(U)$ złączy krzemowych dla polaryzacji przewodzenia', fontsize=12)
